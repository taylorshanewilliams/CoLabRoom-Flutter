...
class SungWrittenLine {
  ...
  /// Sung on no line of its own, but reading as a line that is on the page
  /// and was sung elsewhere in the recording too: a chorus typed once and
  /// sung twice. Shown in place, and never paired with a line nothing was
  /// sung at, because it is not a rewrite of anything.
  final bool sungAgain;
...
/// it was sung in the same breath as.
///
/// The longest run in common will line up an "and" or a "the" across breaths
/// that have nothing else in common, so a breath is only sung at a line where
/// enough of it lines up there: two words, or half of a breath too short to
/// hold two. Anything less and the breath is on no line (review, 18 September
/// 2026: a verse never written down was being absorbed into the short line
/// after it, and offered as that line).
///
/// A breath on no line is shown in place, as sung; a typed line with nothing
/// sung against it is shown as written. Where those two fall together — a
/// line rewritten after the recording — they are paired in order, so the old
/// words sit beside the new ones and can be taken across. But a breath that
/// reads as a line on the page is not a rewrite of anything: where the line
/// it reads as has nothing sung at it, it was sung there, out of its typed
/// order; where the line was sung, this is it sung again, as a chorus typed
/// once and sung twice is (review, 18 September 2026: the second chorus was
/// being paired with the bridge typed after it).
///
...
List<SungWrittenLine> pairSungWithWritten(
  ...
  final breathLength = List<int>.filled(breaths.length, 0);
  ...
  // Forget the alignments too thin to mean the breath was sung at the line:
  // for each breath, the words of it aligned to each line, kept only where
  // there are two of them or they are half of the breath.
  final alignedAt = <(int, int), int>{};
  for (var j = 0; j < sung.length; j += 1) {
    if (sungMatch[j] < 0) continue;
    final at = (breathOf[j], lineOf[sungMatch[j]]);
    alignedAt[at] = (alignedAt[at] ?? 0) + 1;
  }
  for (var j = 0; j < sung.length; j += 1) {
    if (sungMatch[j] < 0) continue;
    final breath = breathOf[j];
    final inCommon = alignedAt[(breath, lineOf[sungMatch[j]])]!;
    if (inCommon >= 2 || inCommon * 2 >= breathLength[breath]) continue;
    writtenMatch[sungMatch[j]] = -1;
    sungMatch[j] = -1;
  }
  ...
  // phrases on no line — one per breath — in the order they were sung.
  ...
  for (var j = 0; j < sung.length; j += 1) {
    ...
    if (phrases.isEmpty || phrases.last.breath != breathOf[j]) {
      phrases.add(_Phrase(breathOf[j], after[j], betweenLines[j]));
    }
    phrases.last.words.add(sung[j]);
  }

  // A phrase that reads as a line on the page — half its words are in that
  // line, in order. It is that line sung, where the line has nothing sung at
  // it yet; otherwise it is the line sung again. Either way it is not a
  // rewrite of the line it happens to fall beside.
  final sungAs = List<_Phrase?>.filled(lines.length, null);
  for (final phrase in phrases) {
    var readsAsALine = false;
    var bestLine = -1;
    var bestInCommon = 0;
    for (var index = 0; index < lines.length; index += 1) {
      final inCommon = _inCommon(writtenByLine[index], phrase.words);
      if (inCommon * 2 < phrase.words.length) continue;
      readsAsALine = true;
      if (taken[index].isNotEmpty || sungAs[index] != null) continue;
    ...
    if (bestLine >= 0) {
      sungAs[bestLine] = phrase;
      phrase.claimed = true;
    } else if (readsAsALine) {
      phrase.sungAgain = true;
    }
  }
  final phrasesAfter = <int, List<_Phrase>>{};
  for (final phrase in phrases) {
    phrasesAfter.putIfAbsent(phrase.after, () => <_Phrase>[]).add(phrase);
  }
  ...
  void flush() {
    ...
    for (var k = pairs; k < unwrittenPhrases.length; k += 1) {
      rows.add(_sungOnly(unwrittenPhrases[k]));
    }
    for (var k = pairs; k < unsungLines.length; k += 1) {
      rows.add(SungWrittenLine(
        line: lines[unsungLines[k]],
        writtenWords: _compared(writtenByLine[unsungLines[k]], null),
  ...
  void placePhrases(int afterLine) {
    for (final phrase in phrasesAfter[afterLine] ?? const <_Phrase>[]) {
      if (phrase.claimed) continue;
      // Sung again, or in a breath of its own in the middle of a line:
      // shown as sung, in its place, and never paired with a later line it
      // was not sung anywhere near.
      if (phrase.sungAgain || !phrase.betweenLines) {
        rows.add(_sungOnly(phrase));
      } else {
  ...
  placePhrases(-1);
  for (var index = 0; index < lines.length; index += 1) {
    final sungHere = sungAs[index];
    if (sungHere == null && taken[index].isEmpty) {
    ...
    rows.add(_beside(
      lines[index],
      writtenByLine[index],
      sungHere?.words ?? <_Word>[for (final j in taken[index]) sung[j]],
    ));
    placePhrases(index);
  }
  flush();
...
/// A typed line beside the words sung at it, compared on their own: which
/// of them are on both sides is settled here, between these two, so a word
/// the song-wide alignment spent elsewhere still reads as shared.
SungWrittenLine _beside(
  Contribution line,
  List<_Word> written,
  List<_Word> sung,
) {
  ...
  return SungWrittenLine(
    line: line,
    writtenWords: _compared(written, writtenMatch),
    sungWords: _compared(sung, sungMatch),
  );
}

SungWrittenLine _sungOnly(_Phrase phrase) => SungWrittenLine(
      line: null,
      writtenWords: const <ComparedWord>[],
      sungWords: _compared(phrase.words, null),
      sungAgain: phrase.sungAgain,
    );

/// The words, with the ones the other side has marked; with no [match] there
/// is no other side, and only a token that is not a word counts as shared.
List<ComparedWord> _compared(List<_Word> words, List<int>? match) =>
    <ComparedWord>[
      for (var i = 0; i < words.length; i += 1)
        ComparedWord(
          words[i].text,
          shared: words[i].key.isEmpty || (match != null && match[i] >= 0),
        ),
    ];

/// How many words the two have in common, in order.
int _inCommon(List<_Word> written, List<_Word> sung) {
  final (writtenMatch, _) = _align(written, sung);
  var count = 0;
  for (final match in writtenMatch) {
    if (match >= 0) count += 1;
  }
  return count;
}
...
class _Phrase {
  _Phrase(this.breath, this.after, this.betweenLines);

  final int breath;

  /// The line it was sung after, or -1 for before the first.
  final int after;
  ...
  /// Taken as the words sung at a line nothing else was sung at: shown there,
  /// not here.
  bool claimed = false;

  /// Reads as a line on the page that was sung elsewhere as well.
  bool sungAgain = false;
}
